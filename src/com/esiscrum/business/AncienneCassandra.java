	...
	public class AncienneCassandra implements EsiscrumService{

		private DbCassandra db;
		public AncienneCassandra() {
			db = new DbCassandra();
		}
		...
		public Project SelectProjectByid(String id) {
			return mapProject(db.select("projet","a_id",id)).get(0);
		}
		...
		public Project SelectCurrentProject(String user) {
			String data[][] =db.select("currentprojet","a_iduser",user);
		...
		public List<Sprint> SelectSprintsByProject(String projet) {
			return mapSprint(db.select("sprint","j_project",projet));
		}
		
		@Override
		public Sprint SelectBySprintid(String id) {
			return mapSprint(db.select("sprint","a_id",id)).get(0);
		}
		...
		public void DeleteSprint(Sprint item) {
			db.delete("sprint", "a_id", item.getId());
		...
		public List<BacklogItem> SelectBacklogsByProject(String projet) {
			...
			return mapBacklog(db.select("backlogitem","l_project",projet));
		}
		
		@Override
		public List<BacklogItem> SelectBacklogsBysprint(String sprint) {
			
			return mapBacklog(db.select("backlogitem","m_sprint",sprint));
		}
		...
		public BacklogItem SelectByBacklogItemid(String id) {
			...
			return mapBacklog(db.select("backlogitem","a_id",id)).get(0);
		}
		...
		public void DeleteBacklogItem(BacklogItem item) {
			...
			db.delete("backlogitem", "a_id", item.getId());
		...
		public List<Task> SelectTaskByBacklog(String idBacklog){
			...
			return mapTask(db.select("task","h_idBacklog",idBacklog)); 
		}
		...
		public Task SelectByTaskid(String id) {
			...
			return mapTask(db.select("task","a_id",id)).get(0);
		}
		...
		public void DeleteTask(Task item) {
			...
			db.delete("task", "a_id", item.getId());
		...
		public Membre SelectByMembreid(String id) {
			...
			return mapMembre(db.select("membre","a_id",id)).get(0);
		}
		
		@Override
		public Membre SelectByMembreUsername(String id) {

			return mapMembre(db.select("membre","d_username",id)).get(0);
		}
		...
		public void DeleteMembre(Membre item) {
			...
			db.delete("membre", "a_id", item.getId());
		...
		public Chat SelectByChatid(String id) {
			return mapMessage(db.select("message","a_id",id)).get(0);
		}
		...
		public void DeleteChat(Chat item) {
			db.delete("message", "a_id", item.getId());
		...
		public void insertBurndown(Burndown b) {
			db.insert("burndown", b.getSprint(),b.getDatemodif(),b.getResteafaire());
		...
		public List<Burndown> SelectBurnDownBySprint(String s) {
			...
			return mapBurndown(db.select("burndown", "a_sprint", s));
		}
		...
		public Burndowninit SelectBurnDownInitBySprint(String s) {
			...
			return mapBurndownInit(db.select("burndowninit", "a_sprint", s));
		}
